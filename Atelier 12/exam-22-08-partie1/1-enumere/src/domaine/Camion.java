...
public class Camion  {
    enum Gabarit {
        DEUX_ESSIEUX(16400, 20), TROIS_ESSIEUX(22000, 20), QUATRE_ESSIEUX(28600, 30);
    ...
    public Camion(String immatriculation, int chargeMaximale, int nbMaxCaisses) {
        this.immatriculation = immatriculation;
        if(chargeMaximale<=Gabarit.DEUX_ESSIEUX.chargeMaximale && nbMaxCaisses<=Gabarit.DEUX_ESSIEUX.nbMaxCaisses)
            this.gabarit = Gabarit.DEUX_ESSIEUX;
        else if(chargeMaximale<=Gabarit.TROIS_ESSIEUX.chargeMaximale && nbMaxCaisses<=Gabarit.TROIS_ESSIEUX.nbMaxCaisses)
            this.gabarit = Gabarit.TROIS_ESSIEUX;
        else if(chargeMaximale<=Gabarit.QUATRE_ESSIEUX.chargeMaximale && nbMaxCaisses<=Gabarit.QUATRE_ESSIEUX.nbMaxCaisses)
            this.gabarit = Gabarit.QUATRE_ESSIEUX;
    }
    ...
    /**
     * renvoie, s'il existe, le premier trajet, dont la date est ultérieure à la date du jour, au quel la caisse
    ...
    private Trajet rechercherTrajet(Caisse caisse) {
        ...
        for (Trajet trajetPrevu : trajets){
            if (trajetPrevu.peutAjouter(caisse)
                && trajetPrevu.nbCaisses() < this.gabarit.nbMaxCaisses
                && trajetPrevu.calculerPoidsTotal() + caisse.getPoids() <= this.gabarit.chargeMaximale) {
