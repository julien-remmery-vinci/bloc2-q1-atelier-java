...
public class Camion  {
    ...
    public List<Trajet> trouverTrajetsAvecPlaceRestante(){
        return trajets.stream().filter(t -> t.nbCaisses() < nbMaxCaisses).collect(Collectors.toList());
    }
    ...
    public LocalDate trouverDateTrajet(String villeDepart, String villeDArrivee){
        return trajets.stream()
                .filter(t -> t.getDate().isAfter(LocalDate.now()) && t.getVilleDepart().equals(villeDepart) && t.getVilleArrivee().equals(villeDArrivee))
